...
- **[Today's Tokens](docs/configuration.md#popup-sections)** - the popup breaks down how many tokens each model used today (output as the headline number, with input and cache read/write on hover), read straight from your local Claude Code transcripts - so you can see usage for models the rate-limit API does not report separately. Fully offline; hide it with `show_token_stats`
